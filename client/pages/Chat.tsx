...
export default function Chat() {
  ...
  const filteredConversations = conversations.filter((conv) => {
    const name =
      conv.type === "dm" ? conv.participants[0]?.displayName : conv.name;
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        ...
                        {conversation.type === "dm" && participant && (
                          <div
                            className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-gray-900 ${getStatusColor(participant.status)}`}
                          ></div>
                        )}
                      </div>
                      ...
                      <div className="relative">
                        ...
                        {currentConversation.type === "dm" &&
                          currentConversation.participants[0] && (
                            <div
                              className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border border-gray-900 ${getStatusColor(currentConversation.participants[0].status)}`}
                            ></div>
                          )}
                      </div>
                          ...
                          {!isOwnMessage && (
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {sender?.rank && sender.rank !== "member" && (
                                <span
                                  className={`text-xs ${getRankColor(sender.rank)}`}
                                >
