...
export default function Friends() {
  ...
  const handleAcceptFriend = (requestId: string) => {
    ...
    if (request) {
      setFriends((prev) => [
        ...prev,
        { ...request.from, friendSince: new Date().toISOString() },
      ]);
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  Manage your friends and see who's playing
                </p>
              </div>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {friendRequests.map((request) => (
                  <div
                    key={request.id}
                    className="card border-yellow-600/30 bg-yellow-600/5"
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className="relative">
                        ...
                        <div
                          className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-gray-800 ${getStatusColor(request.from.status)}`}
                        ></div>
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-xs ${getRankColor(request.from.rank)}`}
                          >
                            {request.from.rank.toUpperCase()}
                          </span>
                        </div>
            ...
            {viewMode === "grid" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredFriends.map((friend) => (
                  <div
                    key={friend.id}
                    className="card hover:scale-[1.02] transition-all duration-200"
                  >
                    <div className="text-center">
                      <div className="relative inline-block mb-4">
                        ...
                        <div
                          className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-gray-800 ${getStatusColor(friend.status)}`}
                        ></div>
                      </div>
                      ...
                      <div className="mb-3">
                        ...
                        <div className={`text-xs ${getRankColor(friend.rank)}`}>
                      ...
                      <div className="text-xs text-gray-500 mb-4">
                        <div>
                          Friends since {formatTimeAgo(friend.friendSince)}
                        </div>
                          ...
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-3">
                              <div className="relative">
                                ...
                                <div
                                  className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border border-gray-800 ${getStatusColor(friend.status)}`}
                                ></div>
                              </div>
                              <div>
                                <div className="flex items-center gap-2">
                                  ...
                                  {getRankIcon(friend.rank)}
                                  <span
                                    className={`text-xs ${getRankColor(friend.rank)}`}
                                  >
                                    {friend.rank.toUpperCase()}
                                  </span>
                                </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 mb-6">
                  {searchTerm
                    ...
                    : "Start building your friend network!"}
                </p>
                <Link
                  to="/friends/requests"
